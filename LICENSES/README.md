...
`NOTICE` file. Other transitive dependencies are tracked by `Cargo.lock`; the
generated inventory and full notices are in `rust-dependencies.json` and
`rust-dependencies.txt`, covering the main, vendor and fuzz lockfiles.
...
Project licensing boundaries are described in [THIRD_PARTY_NOTICES.md](../THIRD_PARTY_NOTICES.md).
The unmodified PolyForm text and legacy sldkit MIT notice are retained in this
directory alongside the dependency notices.
